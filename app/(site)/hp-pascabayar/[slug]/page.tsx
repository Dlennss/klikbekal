...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "HP Pascabayar | KlikBekal",
      description: "Cek dan bayar tagihan HP pascabayar di KlikBekal.",
      path: "/hp-pascabayar",
    });
  }

  return buildPageMetadata({
    title: `${toTitleCase(brand.nama)} | HP Pascabayar KlikBekal`,
    description: `Cek dan bayar tagihan ${toTitleCase(brand.nama)} di KlikBekal dengan proses yang ringkas.`,
    path: `/hp-pascabayar/${slug}`,
    keywords: [`${String(brand.nama).toLowerCase()} pascabayar`, `bayar ${String(brand.nama).toLowerCase()}`, "hp pascabayar", "KlikBekal"],
  });
}
...
export default async function HPPascabayarSlugPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${brandTitle} | HP Pascabayar KlikBekal`,
