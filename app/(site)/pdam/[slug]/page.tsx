...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "Tagihan PDAM | KlikBekal",
      description: "Cek dan bayar tagihan PDAM di KlikBekal.",
      path: "/pdam",
    });
  }

  return buildPageMetadata({
    title: `${brand.nama} | Tagihan PDAM KlikBekal`,
    description: `Cek dan bayar tagihan ${brand.nama} di KlikBekal dengan proses cepat dan ringkas.`,
    path: `/pdam/${slug}`,
    keywords: [`tagihan ${brand.nama.toLowerCase()}`, `bayar ${brand.nama.toLowerCase()}`, "pdam online", "KlikBekal"],
  });
}
