...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "Tagihan PGN | KlikBekal",
      description: "Cek dan bayar tagihan gas PGN di KlikBekal.",
      path: "/pgn",
    });
  }

  return buildPageMetadata({
    title: `${brand.nama} | Tagihan PGN KlikBekal`,
    description: `Cek dan bayar tagihan ${brand.nama} di KlikBekal dengan proses yang ringkas.`,
    path: `/pgn/${slug}`,
    keywords: [`tagihan ${brand.nama.toLowerCase()}`, `bayar ${brand.nama.toLowerCase()}`, "pgn online", "KlikBekal"],
  });
}
